...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	kkal := (runningCaloriesMeanSpeedMultiplier * runningMeanSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return kkal
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkingMeanSpeed := meanSpeed(action, duration) * kmhInMsec //в м/с
	kkal := (walkingCaloriesWeightMultiplier*weight + (math.Pow(walkingMeanSpeed, 2)/height*cmInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return kkal
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	kkal := (meanSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return kkal
}
